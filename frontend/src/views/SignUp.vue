<template>
  <div id="contents">
    <div id="main">
      <section id="pagetop">
        <section class="box">
          <h2 class="title">Sign Up<span>ユーザ登録</span></h2>
          <h2>基本情報をご入力ください</h2>
          <div class="cp_iptxt">
        ...
        <input type="password" v-model="id"  />
        <label>password（confirm）</label>
        <input type="password" v-model="id"  />
        <button v-on:click="subscribe" class="btn btn--yellow btn--cubic">
              Done
        </button> -->
...
export default {
  ...
  methods: {
    signup:function(){
      ...
      params.append("emailadress", this.emailadress);
      axios
        .post("/signup", params)
        .then((response) => {
          alert("登録が完了しました．");
          ...
          this.$router.push({ path: this.$store.state.currentpass });
        })
        .catch((error) => {
